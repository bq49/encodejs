    ...
    request(money) {    // 请求接口
        ...
        Utils.requestFn({ // 重新获取数据
            ...
            success: function (res) {
                console.log('RedPacket request suc res:',res);
                if (res.data.status) {
                    //  返回的支付信息
    ...
    requestPayment(payModel) {
        ...
        wx.requestPayment({
            ...
            'success': function (res) {   // 成功的状态
                //这里支付成功，应该给出一个对话框，提示成功并表示感谢
                // Utils.showModal("支付成功，感谢您的支持");
                console.log("wx.requestPayment suc:",res);
                wx.showModal({
                    title: "",
                    showCancel: false,
                    content: "支付成功，返回上一页",
                    // success:function(res){
                    //     console.log("wx.showModal suc");
                        // wx.navigateBack({
                        //     delta: 2
                        // })
                    // },
                    ...
                    complete:function(res){
                        // console.log('wx.showModal complete');
                        wx.navigateBack({
                            delta: 1
                        })
                    }
